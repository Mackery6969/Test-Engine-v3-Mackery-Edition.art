...
if (fl.version.substring(4).split(",")[0] < 13) {
  ...
  var previewArea = {}
  uri = fl.browseForFileURL(
    "save",
    "where to save files pick folder...",
    previewArea,
    macFormat,
    winFormat
  )
...
for (var i = 0; i < splitArrays.length; i += 1) {
  ...
  if (isJson) fileData += '"frames": ['

  for (
    var frameInd = curLayerInfo[1];
    frameInd < curLayerInfo[2];
    frameInd += 1
  ) {
    if (isJson) fileData += "\n{"
    ...
    for (
      var elementInd = 0;
      elementInd < curFrame.elements.length;
      elementInd += 1
    ) {
      ...
      fileData += addField(
        "alpha",
        curElement.colorAlphaPercent == undefined
          ? 100
          : curElement.colorAlphaPercent,
        false
      )
      fileData += addField("scaleX", curElement.scaleX, false)
      fileData += addField("scaleY", curElement.scaleY, true)
      if (frameInd != curLayerInfo[2] - 1) fileData += "\n"
    }
